...
class MockConnection:
    def __init__(self) -> None:
        self.session= UnifiedAlchemyMagicMock(
            data= [
                (
                    [mock.call.query(PFTable)], # query
                    [PFTable(nome_completo="nome_completo", celular="celular")] # result
                )
            ]
        )
...
def test_list_pf():
    ...
    mock_connection.session.query.assert_called_once_with(PFTable)
